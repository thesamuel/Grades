...
public class Grades {
   ...
   public static final String FILE = "grades.csv";

   public static void main(String[] args) throws FileNotFoundException {
      Scanner console = new Scanner(System.in);
   
      System.out.print("Type a course number (or \"" + SENTINEL + "\" to exit): ");
      String course = console.nextLine();
      while (!course.equals(SENTINEL)) {
         search(course);
         System.out.print("Type a course number (or \"" + SENTINEL + "\" to exit): ");
    ...
    // searches for the specified course in the grades file
   public static void search(String search) throws FileNotFoundException {
      ...
      while (file.hasNextLine()) {
         ...
         if (splits[2].toLowerCase().contains(search.toLowerCase()) 
                    || instructor.toLowerCase().contains(search.toLowerCase())) {
            double gpa = Double.parseDouble(splits[splits.length - 1]);
            String quarter = splits[1].split("[()]")[1];
            System.out.println(formatLine(instructor, gpa, quarter));
            averageGPA += gpa;
            numGPAs++;
         }
      }
    ...
    // formats a line consisting of the course, GPA, and quarter to have uniform length
   public static String formatLine(String instructor, double gpa, String quarter) {
